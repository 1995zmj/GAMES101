        ...
        switch (splitMethod)
        {
            ...
            case SplitMethod::SAH:
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    {
                        auto v = (centroidBounds.Offset(objects[j]->getBounds().Centroid()));
                        int bid = bucketCount * v[i];
...
Intersection BVHAccel::getIntersection(BVHBuildNode* node, const Ray& ray) const
{
    ...
    if (nullptr != node->object && nullptr == node->left && nullptr == node->right)
    {
        intersection = node->object->getIntersection(ray);
    }
    else
    {
        auto leftInter = getIntersection(node->left, ray);
        auto rightInter = getIntersection(node->right, ray);

        intersection = leftInter.distance > rightInter.distance ? rightInter : leftInter;
    }
